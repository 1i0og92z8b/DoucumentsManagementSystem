...
public class StatisticsBizImplTest extends BaseBizTest{
	@Autowired
	IStatisticsBiz iStatisticsBiz;
	
	@Test
	@Rollback(false)
	public void topDownloaderTest(){
		PageCond pc=new PageCond(1,5);
		...
		ur = iStatisticsBiz.topDownloader(pc);
	...
	public void topDownloadedFileTest(){
		...
		df = iStatisticsBiz.topDownloadedFile(pc);
	...
	public void topUploaderByFileCountTest(){
		...
		ur = iStatisticsBiz.topUploaderByFileCount(pc);
		for (UserRankingBo a: ur) {
			System.out.println("-----------New item");
			System.out.println("UserId:  "+a.getUserId());
			System.out.println("UserName:  "+a.getUserName());
	...
	public void topUploaderByDownloadCountTest(){
		...
		ur = iStatisticsBiz.topUploaderByDownloadCount(pc);
		for (UserRankingBo a: ur) {
			System.out.println("-----------New item");
			System.out.println("UserId:  "+a.getUserId());
			System.out.println("UserName:  "+a.getUserName());
	...
	public void recentUploadedFileTest(){
		...
		df = iStatisticsBiz.recentUploadedFile(pc);
		for (DownloadedFileBo a: df) {
			System.out.println("-----------New item");
			System.out.println("FileId:  "+a.getFileId());
			System.out.println("UserId:  "+a.getUserId());
